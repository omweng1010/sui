...
module move_building_blocks::objects {
    ...
    public entry fun freeze_object(object: Object) {
        transfer::freeze_object(object);
    }

    public entry fun create_owned_child(ctx: &mut TxContext) {
        let child = new_child(ctx);
        transfer::transfer(child, tx_context::sender(ctx));
    }

    public entry fun create_owned_children(count: u8, ctx: &mut TxContext) {
        let i = 0;
        while (i < count) {
    ...
    public entry fun wrap_child(object: &mut Object, child: Child) {
        unwrap_child(object);
        option::fill(&mut object.wrapped, child);
    }

    public entry fun unwrap_child(object: &mut Object) {
    ...
    public entry fun unwrap_and_burn_child(object: &mut Object) {
        if (option::is_some(&object.wrapped)) {
            let old_child = option::extract(&mut object.wrapped);
            burn_child(old_child, 0)
        }
    }

    public entry fun table_contains_child(object: &Object, index: u8) {
        let _ = table::contains(&object.table, index);
    }

    public entry fun table_add_child(object: &mut Object, child: Child, index: u8, ctx: &TxContext) {
        table_remove_child(object, index, ctx);
        table::add(&mut object.table, index, child);
    }

    public entry fun table_remove_child(object: &mut Object, index: u8, ctx: &TxContext) {
    ...
    public entry fun table_burn_child(object: &mut Object, index: u8) {
        if (table::contains(&object.table, index)) {
            let child = table::remove(&mut object.table, index);
            burn_child(child);
        }
    ...
    fun burn_child(child: Child, dice: u8) {
        if (dice % 5 == 0) {
            let Child { id } = child;
            object::delete(id);
        }
